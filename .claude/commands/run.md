...
2. Drive the resolved flow doc (`flow/<flow_name>.md` or the matching `tests/<category>/<flow_name>.md`) by delegating to the `flow-runner` agent. `flow-runner` handles the compiled-plan lifecycle itself (`compilePlan`'s `check` → compile on a cache miss, same process as the `flow-compiler` agent → deterministic `run-plan` → scoped local recovery on divergence) — nothing extra to orchestrate here. This includes `report_tool.sh start`/`end` around the run — see `generateReport`'s SKILL.md.
